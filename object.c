...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    memcpy(object_data, header, (size_t)header_len);
    ...
    while (written < object_len) {
        ssize_t n = write(fd, object_data + written, object_len - written);
        if (n <= 0) {
            close(fd);
            unlink(tmp_path);
            free(object_data);
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    char type_str[16];
    size_t declared_len;
    if (sscanf((const char *)buffer, "%15s %zu", type_str, &declared_len) != 2) {
        free(buffer);
        return -1;
    }
    ...
    size_t header_len = (size_t)(nul - buffer) + 1;
    if ((size_t)file_size != header_len + declared_len) {
        free(buffer);
        return -1;
    }

    void *data = malloc(declared_len == 0 ? 1 : declared_len);
    if (!data) {
        free(buffer);
        return -1;
    }

    if (declared_len > 0) memcpy(data, buffer + header_len, declared_len);
